...
$dashboardLink = ($userRol === 'empresa') ? 'dashboard_empresa.php' : 'mis_postulaciones.php';
?>
<!DOCTYPE html>
<html lang="es">
...
<body class="hero-bg min-h-screen text-white flex flex-col">
    ...
    <main
        ...
        <div class="w-full max-w-4xl mx-auto mb-12 relative z-10">
            <form action="empleos.html" method="GET"
                ...
                <div class="flex-1 w-full relative group">
                    ...
                    <input type="text" name="q" id="inputQuery" autocomplete="off"
                        placeholder="¿Qué estás buscando? (Ej: Clarkista)"
                        class="w-full bg-transparent text-white placeholder-slate-500 pl-12 pr-4 py-4 rounded-2xl focus:outline-none focus:bg-white/5 transition relative z-20">

                    <!-- Live Search Dropdown -->
                    <div id="searchSuggestions"
                        class="hidden absolute top-full left-0 w-full mt-2 bg-[#002b4d] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden backdrop-blur-xl">
                        <div id="suggestionsList" class="divide-y divide-white/10"></div>
                        <a href="#" onclick="document.querySelector('form').submit()"
                            class="block w-full py-3 bg-[#001f38] text-center text-xs font-bold text-white uppercase tracking-widest hover:bg-[#001529] transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <a href="empleos.html?cat=deposito"
                    ...
                    <div class="text-left">
                        ...
                        <p class="text-slate-500 text-xs">Carga, Clarkistas</p>
                    </div>
                    <div class="ml-auto md:hidden text-slate-500">
                        <i class="fas fa-chevron-right"></i>
                    </div>
                ...
                <a href="empleos.html?cat=admin"
                    ...
                    <div class="text-left">
                        <h3 class="text-white font-bold text-lg group-hover:text-purple-300 transition">Admin
                        </h3>
                        <p class="text-slate-500 text-xs">Analistas, RRHH</p>
                    </div>
                    <div class="ml-auto md:hidden text-slate-500">
                        <i class="fas fa-chevron-right"></i>
                    </div>
                ...
                <a href="empleos.html?cat=tecnico"
                    ...
                    <div class="text-left">
                        <h3 class="text-white font-bold text-lg group-hover:text-orange-300 transition">Técnico</h3>
                        <p class="text-slate-500 text-xs">Mecánicos, Mant.</p>
                    </div>
                    <div class="ml-auto md:hidden text-slate-500">
                        <i class="fas fa-chevron-right"></i>
                    </div>
        ...
        <div class="grid md:grid-cols-2 gap-4 w-full max-w-2xl mx-auto mb-12 opacity-80 hover:opacity-100 transition">
            <a href="login_candidato.html"
                class="glass p-4 rounded-xl flex items-center gap-4 hover:bg-white/5 transition group">
                <div
                    class="w-10 h-10 bg-slate-700 rounded-full flex items-center justify-center text-slate-300 group-hover:bg-emerald-500 group-hover:text-white transition">
            ...
            <a href="login_empresa.html"
                class="glass p-4 rounded-xl flex items-center gap-4 hover:bg-white/5 transition group">
                <div
                    class="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-slate-300 group-hover:bg-cyan-500 group-hover:text-white transition">
    ...
    <script>
        ...
        function renderSuggestions(items, query) {
            suggestionsList.innerHTML = items.map(item => `
                <a href="empleos.html?q=${encodeURIComponent(item.titulo)}" class="flex items-center justify-between px-4 py-3 hover:bg-emerald-500/10 group transition cursor-pointer border-b border-white/5 last:border-0">
                    <div class="flex items-center gap-3">
                         <div class="w-8 h-8 rounded bg-white/5 border border-white/10 flex items-center justify-center">
                            <i class="fas fa-search text-xs text-slate-400 group-hover:text-emerald-400 transition"></i>
                         </div>
                         <div class="text-left">
                            <p class="text-sm font-bold text-white leading-none mb-1 group-hover:text-emerald-300 transition">${item.titulo}</p>
                            <p class="text-[10px] text-slate-400 uppercase tracking-wide flex items-center gap-1">
                                <i class="fas fa-map-marker-alt text-[8px]"></i> ${item.ubicacion || 'Ubicación n/a'}
                            </p>
                         </div>
                    </div>
                    <i class="fas fa-arrow-right text-xs text-slate-600 group-hover:text-emerald-400 -translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition"></i>
                </a>
            `).join('');
        }
        ...
        document.addEventListener('click', function (e) {
            if (!inputQuery.contains(e.target) && !suggestionsBox.contains(e.target)) {
                suggestionsBox.classList.add('hidden');
            }
        });
    </script>
