...
if ($action === 'check') {
    ...
    if (isset($_SESSION['user_id'])) {
        // Force Refresh Role from DB (Fix for manual role changes)
        $stmtRol = $pdo->prepare("SELECT rol FROM usuarios WHERE id = ?");
        $stmtRol->execute([$_SESSION['user_id']]);
        $currentRol = $stmtRol->fetchColumn();
        if ($currentRol) {
            $_SESSION['user_rol'] = $currentRol;
        }
...
if ($action === 'logout') {
    ...
    if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
        ...
        // Direct link access -> Redirect to Home
